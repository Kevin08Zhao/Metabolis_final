# Metabolis Final Locked Palette

`art/palette.gpl` and this table together are the final 22-color source produced by D-01 and adjusted by D-02. Reference colors by the hex values in this file; never resample them from screenshots.
...
| `#140F1D` | The single project-wide exterior outline and deepest occlusion seam | Resource fill, object-specific colored outlines |
| `#514854` | Non-semantic shadow, interior background, deep UI layer | Replacing the global outline; encoding waste or a resource |
| `#817582` | Non-semantic mid layer, UI frame, secondary structure | Resource quantity, transport direction, stability state |
| `#E8DCCF` | Non-semantic highlight, text, shared specular point | Replacing knowledge blue, completion mint, or energy amber |

The three displayed stability tiers use `#B1FFD1`, `#E2953A`, and `#BA3A3F` respectively. Continuous stability is encoded by fill length and must not generate intermediate colors outside the palette.
